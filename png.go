...
func NewPngChunkSlice() *ChunkSlice {
	...
	ihdrChunk := &Chunk{
		...
		// TODO(dustin): !! Finish.
		// TODO(dustin): !! Are there any other Chunk{} creations that don't populate Crc?
		Crc: uint32(0),
	}
...
func (cs *ChunkSlice) SetExif(ib *exif.IfdBuilder) (err error) {
	...
	if err == nil {
		...
		chunk.Data = exifData
		chunk.Length = uint32(len(exifData))
	} else {
		...
		// put it).
		cs.chunks = append(cs.chunks[:1], append([]*Chunk{chunk}, cs.chunks[1:]...)...)
	}
...
// TODO(dustin): !! Still need to check/set CRC values.
